...
- **Cons**: Minimal security; no refresh token, no revocation list, replay feasible during 1-hour TTL.  
  _How to harden later_: add refresh token & revocation list, consider explicit CSRF tokens if SameSite mode ever needs to be relaxed, and evaluate shorter TTL + silent token refresh. Out of scope: DPoP, mTLS.
