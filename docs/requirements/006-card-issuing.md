...
| FR-CARD-4  | Card Reveal: Full PAN and CVC are returned only at issuance; subsequent reads return masked PAN (last4) and no CVC.                                                                                                  | Must     |
| FR-CARD-5  | UI Cards Panel: The web app provides a Cards section per account that lists cards with last4, expiry, status, and an Issue Card action that shows the full card details once.                                        | Must     |
...
| FR-CARD-12 | Demo Data Handling: Raw PAN/CVC are never stored or logged; cards and card-related items written for `isTest=true` users include a TTL.                                                                              | Must     |
...
| ---------- | ------------- | ----------------------------------------------------------------------------------------------- | ----------------------------- |
| NFR-CARD-1 | Security      | No raw PAN or CVC stored or logged; use HMAC hash for lookup and masked values for display.     | 0 raw PAN/CVC in storage/logs |
...
- Issuing: User signs in, issues a card for an account, and receives PAN (BIN
  123456), expiry, and CVC once; subsequent card reads show only last4 and no
  CVC. Multiple cards can be issued for the same account.
