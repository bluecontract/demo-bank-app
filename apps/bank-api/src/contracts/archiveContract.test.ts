...
import { archiveContractHandler } from './archiveContract';
...
describe('archiveContractHandler', () => {
  const logger = {
    info: vi.fn(),
  };
  ...
  it('returns 404 when contract is missing', async () => {
    ...
    const response = await archiveContractHandler(
      {
  ...
  it('archives the contract for the current user', async () => {
    ...
    expect(response.body).toEqual({
      status: 'ok',
      myosStatus: 200,
      body: { archivedAt: now.toISOString() },
    });
  });
});
