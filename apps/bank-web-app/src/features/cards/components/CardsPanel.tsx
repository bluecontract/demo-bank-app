            ...
            {cards && cards.length > 0 ? (
              cards.map(card => (
                <button
                  key={card.cardId}
                  type="button"
                  className="flex items-center justify-between bg-white/70 border border-slate-200 rounded-2xl p-4 shadow-sm text-left transition hover:border-emerald-200 hover:shadow-md"
                  onClick={() => handleCardClick(card)}
                  aria-label={`View details for card ending ${card.panLast4}`}
                >
                  <div>
