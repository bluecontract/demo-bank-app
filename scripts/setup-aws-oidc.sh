...
echo "DEV_FRONTEND_URL=\$(aws cloudformation describe-stacks --stack-name demo-bank-app-frontend-dev --query 'Stacks[0].Outputs[?OutputKey==\`FrontendUrl\`].OutputValue' --output text --region ${AWS_REGION})"
echo "# For prod environment:"
echo "PROD_FRONTEND_URL=\$(aws cloudformation describe-stacks --stack-name demo-bank-app-frontend-prod --query 'Stacks[0].Outputs[?OutputKey==\`FrontendUrl\`].OutputValue' --output text --region ${AWS_REGION})"
echo ""
