...
/**
 * Route map aligned to the current bankApiContract in the full repo.
 * Keep names consistent with sessionId-oriented routes.
 */
...
export class BankTestDriver {
  async signUpUniqueTestUser(
    ...
    return { userId: response.body.userId, jwtCookie, userEmail, merchantName };
  }
  ...
  async createFundedAccount(
    ...
    assertSafeTestAmount(
      Math.min(fundingAmountMinor, FAST_AMOUNTS.fundingBufferMinor)
    );
  ...
  async waitForDeliveryBySessionId(
    ...
    await waitForExpectWithLogging(
      async () => {
        const response = await this.getPayNoteDeliveryBySessionId(
          jwtCookie,
          sessionId
        );
